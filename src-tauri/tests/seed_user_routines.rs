...
async fn seed_and_verify_user_routines() {
    ...
    let routines = db
        ...
        .await
        .expect("list routines after seed");
    assert_eq!(
        routines.len(),
    ...
    let habits = db
        .list_habits(user_id)
        .await
        .expect("list habits after seed");
    assert!(
        habits
            .iter()
            .any(|habit| habit.title == "Junk food" && habit.active),
    ...
    println!(
        "SEED_VERIFY_OK user={USER_EMAIL} routines={} habits={}",
        routines.len(),
        habits.len()
    );
    println!(
        "DAILY_LOG_DEFAULT trading_profit={:?} book_title={:?} water_ml={:?}",
        today.daily_log.trading_profit, today.daily_log.book_title, today.daily_log.water_ml,
    );
}
